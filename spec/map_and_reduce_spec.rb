...
describe 'my own reduce-like methods' do
  ...
  describe "reduce_to_total1 returns a running total when given a starting point" do
    it "reduces correctly" do
      source_array = [1,2,3]
      starting_point = 100
      expect(reduce_to_total1(source_array, starting_point)).to eq(106)
    end
  end
  ...
  describe "reduce_to_any_true1 returns false when no truthy value is present" do
    it "reduces correctly" do
      source_array = [ false, nil, nil, nil]
      expect(reduce_to_any_true1(source_array)).to eq(false)
    end
  end
end
